...
c***********************************************************************
  ...
  35    format (i5)
        nbl = nbl - 1
        if (nbl*2.gt.idim2) then
          write (*,*) 'ERROR:  idim2 is too small'
          stop
        end if
        do i = 0,nbl,1
...
c      call COSFT(us,nbl2,-1)
c      call COSFT(d1us,nbl2,-1)
c      call COSFT(d2us,nbl2,-1)
c      do i = 0, nbl2
c        write (10,50) xi(i),us(i),d1us(i),d2us(i)
...
#else
      write(*,*) "LAPACK Adjoint not yet implemented, use IMSL"
c     Need LAPACK here...
#endif 
c
      do i = 0, N-2 
...
c***********************************************************************
      ...
      COMPLEX   T(0:NBIG), DT(0:NBIG), ALPHA, C, max
      ...
      DO J = 0, NBIG
        ...
        END DO
        T(J) = CMPLX(TR,TI)
        if ( ABS(T(J)) .gt. ABS(max) ) then
          max = T(j)
        end if
C       XI = COS(Y(J))*LMAP/2.+LMAP/2.+YMIN
C       WRITE (20,10) XI, TR, TI
      END DO

      DO J = 0, NBIG
        XI = COS(Y(J))*LMAP/2.+LMAP/2.+YMIN
        T(J) = T(J)/max
        WRITE (20,10) XI, REAL(T(J)), AIMAG(T(J))
      END DO
...
C
      ...
      write (10) NBIG+1, NBIG+1
      write (10) (( X(I), i=0,NBIG), j=0,NBIG),
     .           (( COS(Y(J))*LMAP/2.+LMAP/2.+YMIN,i=0,NBIG),j=0,NBIG)

      open (unit=11,file='q.dat',form='unformatted',status='unknown')
      write (11) NBIG+1, NBIG+1
      write (11) 0.0,0.0,RE,TIME
      write (11)  ((PSIR(i,j), i=0,NBIG), j=0,NBIG),
     .            ((   U(i,j), i=0,NBIG), j=0,NBIG),
     .            ((   V(i,j), i=0,NBIG), j=0,NBIG),
     .            ((PSII(i,j), i=0,NBIG), j=0,NBIG)
