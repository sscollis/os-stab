...
c               finite-difference method for boundary layer profiles
c               using algebraic mapping.
c
...
c***********************************************************************
      ...
      integer     LWORK
      complex     WORK(2*(idim+1))
      real        RWORK(2*(idim+1))
      integer     INFO, LDVL
      complex     VL(0:idim,0:idim)
C***********************************************************************
      lda = idim+1
      ldb = idim+1
      ldevec = idim+1
...
c     Must watch out, this routine isn't very robust.
 ...
 100    format (/,1x,'Residual = ',f17.10)
        ...
        write (*,32)
        do i = 1, N
          write (*,35) REAL(eval(i)/alpha), AIMAG(eval(i)/alpha), i
